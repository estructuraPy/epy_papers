...
def test_it_remembers_in_this_application_scope(
    ...
    dialog._combo_doctype.setCurrentText("paper")
    dialog.persist_settings()
    stored = scratch_settings(ORGANIZATION, "epy_papers")
    assert stored.value("docs_doctype") == "paper"
    assert not (tmp_path / f"{ORGANIZATION}__epy_slides.ini").exists()
...
def test_the_worker_reaches_this_application_bridge(
    ...
    ded._RenderWorker(
        tmp_path / "articulo.md", "academic", "paper", tmp_path / "out",
        True, False,
    ).run()
