...
Direction of record for `epy_papers`, kept beside `CHANGELOG.md`: this file
...
`epy_papers` produces papers. It is one of the editors `epy_studio` hosts,
alongside `epy_reports` and `epy_slides` and `epy_draft`, and they share the export
...
### Optional autosave (2026-09-01, shipped 046ca13 on 2026-09-02)
...
built: a paper that has never been saved is **skipped**, because
...
Found and fixed alongside it: `Paper.to_draft` ran on the GUI thread, so
a PDF export froze the window for the whole LaTeX compile — the
application looked hung and Windows offered to close it. It runs on a
worker thread now, behind the busy dialog the TinyTeX download already
used, and the same counter covers both nested event loops.
