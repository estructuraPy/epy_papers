...
Output:
    dist/epy-papers_<version>_all.deb
...
DESCRIPTION_LONG = """\
  ...
  * 50 journal profiles including ASCE, Elsevier, and Springer families
  * Live preview rendered by pandoc
  * CSL citation style support (14 styles including ASCE)
  * YAML front-matter for journal selection, authors, and metadata
...
# pandoc is provided by apt so the correct architecture-native binary
...
def _tar_add_dir(
    tf: tarfile.TarFile, arcname: str, mode: int = 0o755
) -> None:
...
def _build_control_tar() -> bytes:
    ...
    with tarfile.open(fileobj=buf, mode="w:gz") as tf:
        ...
        postinst = textwrap.dedent("""\
            ...
            "$VENV/bin/pip" install PySide6 PyYAML pypandoc

            # Update system MIME database so .md type is recognized.
...
from epy_papers.app import main; sys.exit(main())" "$@"
        ...
        for path in sorted(PYPANDOC_SRC.rglob("*")):
            ...
            arcname = (
                f"./usr/lib/{PKG_NAME}/pypandoc/{rel}".replace("\\", "/")
            )
            if path.is_dir():
                _tar_add_dir(tf, arcname)
            else:
        ...
        # MIME type definition (text/markdown is standard; this entry
        # ensures .md is registered even on minimal systems).
        mime_xml = textwrap.dedent("""\
            ...
            <mime-info xmlns="http://www.freedesktop.org/standards/shared-mime-info">
              <mime-type type="text/markdown">
                ...
                <acronym>MD</acronym>
                <expanded-acronym>Markdown Document</expanded-acronym>
                <glob pattern="*.md"/>
        ...
        if png_path.exists():
            ...
            print(
                f"  WARNING: {png_path} not found — "
                "icon will be missing from .deb.",
                file=sys.stderr,
            )
