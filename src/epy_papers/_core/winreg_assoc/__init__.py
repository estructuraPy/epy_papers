...
* ``register(make_default=False)`` — advertise the app in HKCU so it
  appears under "Open with..." for ``.md`` / ``.markdown``
...
``Software\epy_papers`` and ``Software\RegisteredApplications``), so
``unregister`` removes them without touching anything else.

Mirrors ``epy_reports._core.winreg_assoc`` with the epy_papers identity
and its supported extensions (no ``.qmd`` — the paper editor opens
plain Markdown manuscripts).
"""
...
PROGID = "epy_papers.Document.1"
APP_NAME = "epy_papers"
APP_DESCRIPTION = (
    "Academic manuscript editor with journal templates."
)
APP_KEY = f"Applications\\{APP_NAME}.exe"
EXTENSIONS = (".md", ".markdown")
EXT_DESCRIPTION = "Markdown Document"
...
def _launcher_path() -> str:
    """Locate the installed ``epy_papers`` launcher executable.

    Falls back to ``python -m epy_papers`` if no console script was
...
def register(make_default: bool = False) -> list[str]:
    """Register epy_papers for ``.md`` / ``.markdown`` in HKCU.
